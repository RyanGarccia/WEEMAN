...
class handler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        post_request = []
        printt(3, "%s - Pedido de postagem enviada." %self.address_string())
    ...
    def log_message(self, format, *args):
        ...
        arg = format%args
        if arg.split()[1] == "/":
            printt(3, "%s - Enviado Obter pedido sem parametros." %self.address_string())
        else:
            if arg.split()[1].startswith("/") and "&" in arg.split()[1]:
                printt(3, "%s - Enviado Obter pedido com parametros." %self.address_string())
...
class weeman(object):
    ...
    def serve(self):
        ...
        print("\033[01;35m[i] Iniciando o Weeman %s servidor em http://localhost:%d\033[00m" %(__version__, self.port))
...
def create_post(url,action_url, post_request):
    ...
    printt(3, "Criando redirect.html ...")
